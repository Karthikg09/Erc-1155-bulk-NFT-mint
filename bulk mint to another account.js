...
(async () => {
    try {
        const totalNfts = 145;      //specify the total number of nfts
        const nftOwnerAddress = 'ADDRESS_OF_ACCOUNT_TO_MINT_TO';
        ...
        for (let i = startTokenID; i <= maxTokenID; i += mintSize) {
            ...
            await test.methods
            .mintBatchTo(nftOwnerAddress, ids, amounts)
            .send({ from: myAddress });
        ...
        const balance = await test.methods
            .balanceOf(nftOwnerAddress, 1)
            .call();
